...
public class PaperweightRegen extends Regenerator {
    ...
    protected boolean initNewWorld() throws Exception {
        ...
        if (FoliaUtil.isFoliaServer()) {
            return initWorldForFolia();
        }

        return true;
    }

    private boolean initWorldForFolia() throws ExecutionException, InterruptedException {
        MinecraftServer console = ((CraftServer) Bukkit.getServer()).getServer();

        // Minecraft 26.1 selects the world spawn internally in initWorld -> setInitialSpawn,
        // deriving the same chunk from findSpawnPosition. We only need spawnChunk to run
        // initWorld on the region thread that owns the spawn area.
        ...
        Bukkit.getServer().getRegionScheduler().run(
                ...
                task -> {
                    try {
                        console.initWorld(freshWorld, null);
